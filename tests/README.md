...
| [integration/zsh/zsh-plugins.bats](integration/zsh/zsh-plugins.bats)                               | Completion discovery, plugin loading order, inherited and queried prefixes, missing plugin diagnostics, and arrow bindings.    |
...
| [e2e/zsh-plugins.bats](e2e/zsh-plugins.bats)                                                       | One completion initialization and real interactive completion, directory jumps, suggestions, highlighting, and history search. |
...
`setup.bats` starts with no chezmoi, Homebrew, or Oh My Zsh in a fresh Ubuntu container. Root only provisions a test account with sudo access; that ordinary user follows the README prerequisites and installation steps. It checks that Brewfile dependencies are satisfied and installed package versions remain unchanged after a second apply. `NONINTERACTIVE=1` answers Homebrew's unattended-installation prompts.
